...
BTreeNode *search(BTreeNode *root, int IDProduk, int *index); //penelusuran node dengan identifier tertentu
...
void AdjustNode(BTreeNode *node, int index); //children dari node pada posisi tertentu memiliki item minimum
...
BTreeNode *search(BTreeNode *root, int IDProduk, int *index){
    ...
    return search(root->children[i], IDProduk, index);
}
...
int main() {
    ...
    printf("\nMencari produk dengan ID Produk 3245:\n");
    int index = -1;
    BTreeNode *HasilSearch = search(root, 3245, &index);
    if (HasilSearch && index != -1) {
        printf("Produk ditemukan:\nID Produk: %d\nNama Produk: %s\n", HasilSearch->keys[index].IDProduk, HasilSearch->keys[index].NamaProduk);
    } else {
        printf("Produk tidak ditemukan\n");
    }

    printf("\nMenghapus produk dengan ID Produk 1245\n");
    Delete_Key(root, 1245);
